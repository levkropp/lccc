...
- **~1.34× of GCC -O2** across the 16 conventional benchmarks (0.64× geomean including the
  two ~500× recursion-to-iteration wins)
- **matmul at parity** (1.01×), **bitops 1.7× faster**, **qsort/tce_sum ~1.0–1.13×**
...
| `fannkuch` | 1.94× | Continue register-pressure work in the flip loop |
| `loop_patterns` | 1.83× | Eliminate NEON accumulator spills in reduction loops |
| `nbody` / `mandelbrot` | ~1.65× | FP codegen: fewer GPR↔FP round-trips, better scheduling |
| `sieve` | 1.53× | Memory-write loop tightening |
| Correctness | 6 + 4 suite failures | Struct passing, designated initializers, multi-dim arrays |
...
systems software, targeting within ~1.5× of GCC on typical workloads.
...
| Recent | AArch64: NEON vector ops, F64 loop promotion, register steal, loop-backedge slot coalescing, int-widen register casts, several latent miscompile fixes | Geomean 0.86× → 0.64× of GCC |
